...
import { invalidLoginTypeMock, loginMockUser, loginUserMock, userMock, wrongPasswordMock } from "../../__mock__/userMocks";
...
describe("Integration test: Login User", () => {
    ...
    test("Should be able to login successfully", async () => {
        ...
        const data = await request.post("/users/login").send(loginUserMock).expect(200).then((response) => response.body)
    ...
    test("Should throw error when user is not registered", async () => {
        ...
        const data = await request.post("/users/login").send(loginUserMock).expect(404).then((response) => response.body)
    ...
    test("Should thrown an error when credentials do not match", async () => {
        ...
        const data = await request.post("/users/login").send(wrongPasswordMock).expect(401).then((response) => response.body)
    ...
    test("Should thrown an error when body parameter is missing", async () => {
        ...
        const data = await request.post("/users/login").expect(400)
    ...
    test("Should throw error when body parameter receives wrong data type", async () => {
        ...
        const data = await request.post("/users/login").send(invalidLoginTypeMock).expect(400).then((response) => response.body)
