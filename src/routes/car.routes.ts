...
carRouter.post(
  "/",
  VerifyToken.execute,
  ValidateBody.execute(createCarSchema),
  (req, res) => carControllers.createCar(req, res)
);
...
carRouter.patch(
  ...
  (req, res) => carControllers.updateCar(req, res)
);
carRouter.delete("/:id", VerifyToken.execute, (req, res) =>
  carControllers.deleteCars(req, res)
);
